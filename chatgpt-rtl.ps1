...
'@
    ...
    # every build so the exe path / identity stay current.
    $identity = Get-SourceIdentity $srcDir
    $exePath  = $exe.FullName.Replace("'", "''")
    if ($identity) {
...
`$exe = '$exePath'
try {
  Invoke-CommandInDesktopPackage -PackageFamilyName '$fam' -AppId '$aid' -Command `$exe -PreventBreakaway
} catch {
  # Documented external-launch fallback (cmd proxy), then a bare launch.
  try { Invoke-CommandInDesktopPackage -PackageFamilyName '$fam' -AppId '$aid' -Command 'cmd.exe' -Args ('/c ""' + `$exe + '""') -PreventBreakaway }
  catch { Start-Process `$exe }
}
"@
    } else {
      # Non-MSIX install (Squirrel/NSIS): no package identity needed.
      $launcher = "Start-Process '$exePath'"
    }
